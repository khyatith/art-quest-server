...
function getLeaderboard(room) {
  ...
  if (englishAuctionsObj) {
    for (var englishAuction in englishAuctionsObj) {
      ...
      if (leaderBoardKeys && leaderBoardKeys.includes(EAwinningTeam)) {
        const isExistingAuction = leaderboard[EAwinningTeam].filter(
          (item) =>
            parseInt(item.auctionId, 10) === parseInt(EAWinner.auctionId, 10)
        )[0];
  ...
  if (englishAuctionsObj2) {
    for (var englishAuction2 in englishAuctionsObj2) {
      ...
      if (leaderBoardKeys && leaderBoardKeys.includes(EAwinningTeam2)) {
        const isExistingAuction = leaderboard[EAwinningTeam2].filter(
          (item) =>
            parseInt(item.auctionId, 10) === parseInt(EAWinner2.auctionId, 10)
        )[0];
        if (!isExistingAuction) {
  ...
  if (englishAuctionsObj3) {
    for (var englishAuction3 in englishAuctionsObj3) {
      ...
      if (leaderBoardKeys && leaderBoardKeys.includes(EAwinningTeam3)) {
        const isExistingAuction = leaderboard[EAwinningTeam3].filter(
          (item) =>
            parseInt(item.auctionId, 10) === parseInt(EAWinner3.auctionId, 10)
        )[0];
        if (!isExistingAuction) {
  ...
  if (dutchAuctionObj) {
    for (var dutchAuction in dutchAuctionObj) {
      ...
      if (leaderBoardKeys && leaderBoardKeys.includes(DAwinningTeam)) {
        const isExistingAuction = leaderboard[DAwinningTeam].filter(
          (item) =>
            parseInt(item.auctionId, 10) === parseInt(DAWinner.auctionId, 10)
        )[0];
        if (!isExistingAuction) {
  ...
  if (firstPricedSealedBidAuctionsObj) {
    for (var fristPricedSealedAuction in firstPricedSealedBidAuctionsObj) {
      ...
      if (leaderBoardFSBKeys && leaderBoardFSBKeys.includes(FPSBwinningteam)) {
        const isExistingFPSBAuction = leaderboard[FPSBwinningteam].filter(
          (item) => item.auctionId === FPSBwinner.auctionId
        )[0];
  ...
  if (secondPricedSealedBidAuctionObj) {
    for (var secondPricedSealedAuction in secondPricedSealedBidAuctionObj) {
      ...
      const SPSBItem =
        secondPricedSealedBidAuctionObj[secondPricedSealedAuction];
      ...
      if (
        leaderBoardSPSBKeys &&
        leaderBoardSPSBKeys.includes(SPSBwinningteam)
      ) {
        const isExistingSPSBAuction = leaderboard[SPSBwinningteam].filter(
          (item) => item.auctionId === SBSPWinnerFinal.auctionId
        )[0];
  ...
  if (allPayAuctionBidObj) {
    for (var allPayAuctionBids in allPayAuctionBidObj) {
      ...
      if (
        leaderBoardAllAuctionKeys &&
        leaderBoardAllAuctionKeys.includes(allAuctionwinningteam)
      ) {
        const isExistingAllAuction = leaderboard[allAuctionwinningteam].filter(
          (item) => item.auctionObj.id === allAuctionwinner.auctionId
        )[0];
...
async function calculateTotalAmountSpent(leaderboard, currentRoom) {
  const allPayAuctionBidObj = currentRoom.allPayAuctions;
  let result;
  ...
  if (totalAmt.length !== 0 && Object.keys(totalAmt[0]).length !== 0) {
    result =
      ...
      totalAmt.reduce(
        (obj, item) =>
          Object.assign(obj, {
            [item.key]: currentRoom.startingBudget - item.value,
          }),
        {}
      );
  }
  ...
  if (Object.keys(allPayAuctionBidObj).length > 0) {
    const allPayAuctionAmt =
      updateTotalAmountsForAllPayAuctions(allPayAuctionBidObj);
    if (result) {
      result = Object.entries(result).reduce((acc, [key, value]) => {
        let total = 0;
        if (value) {
          total = allPayAuctionAmt[key]
            ? parseInt(value) + allPayAuctionAmt[key]
            : parseInt(value);
        } else {
          total = allPayAuctionAmt[key];
        }
...
function updateDutchAuctionLeaderboard(room) {
  ...
  if (Object.keys(dutchAuctionBids).length > 0) {
    for (var dutchAuction in dutchAuctionBids) {
      ...
      if (leaderBoardKeys && leaderBoardKeys.includes(DAwinningTeam)) {
        const isExistingDAAuction = leaderBoard[DAwinningTeam].filter(
          (item) => item.auctionId === auctionItem.auctionId
        )[0];
...
function getNextObjectForLiveAuction(parsedRoom, prevAuctionId) {
  ...
  if (!parseInt(prevAuctionId)) {
    ...
    const nextId = id + 1;
    newAuction = parsedRoom.auctions.artifacts.filter(
      (item) => item.id === nextId
    )[0];
    //update parsed rooms
...
function calculateBuyingPhaseWinner(room) {
  ...
  return {
    leaderBoard,
    totalPaintingsWonByTeam: totalPaintingsWonByTeam,
    allTeams,
    teamEfficiency,
    totalAmountSpentByTeam,
  };
  ...
  for (team in leaderboard) {
    ...
    const teamEfficiency =
      currentTeamData.length > 0
        ? parseFloat(
            parseInt(
              totalAmountByTeam[team] / parseFloat(currentTeamData.length)
            )
          )
        : 0;
    efficiencyByTeam = {
...
function calculateSellingRevenue(data) {
  ...
  let revenue =
    (parseFloat(interestInArt) * parseFloat(5.0)) /
    (parseFloat(population) +
      parseFloat(ticketPrice) +
      parseInt(allTeamsInCity)); //paintingQuality is replaced with fixed value 5.0
  if (isInt(revenue)) {
    return revenue;
  } else {
...
function createTeamRankForBuyingPhase(
  totalPaintingsWonByTeam,
  teamEfficiency,
  ...
  let index = 10;
  const totalEfficiencyByTeam = Object.keys(sortedByEfficiency).reduce(
    (acc, team) => {
      acc = {
        ...acc,
        [team]: index,
  ...
  const teamScores = Object.entries(totalPaintingsWonByTeam).reduce(
    (acc, [key, value]) => {
      const currentTeam = key;
      const teamScore =
        (value * 50 + totalEfficiencyByTeam[currentTeam] * 50) /
        (totalNumberOfPaintings + 10) /
        2;
      acc = {
        ...acc,
...
function getWinningEnglishAuctionBid(
  ...
  let EAWinner = {};
  if (
    !maxEnglishAuctionBids[`${auctionId}`] &&
    (!highestBidObj || Object.keys(highestBidObj).length === 0)
  )
    return {};
  if (!maxEnglishAuctionBids[`${auctionId}`]) {
    return {
      EAWinningTeam: highestBidObj.bidTeam,
      EAWinnerBid: highestBidObj.bidAmount,
      highestBidObj,
    };
  }
  const allBidsArr = maxEnglishAuctionBids[`${auctionId}`].map((obj) =>
    parseInt(obj.bidAmount)
  );
  const highestBidInMaxAuctionBidsArray =
    allBidsArr.length === 1 ? allBidsArr[0] : findHighestBid(allBidsArr);
  let winner = maxEnglishAuctionBids[`${auctionId}`].filter(
    (obj) =>
      parseInt(obj.bidAmount) === parseInt(highestBidInMaxAuctionBidsArray)
  );
  if (highestBidObj && Object.keys(highestBidObj).length > 0) {
    if (highestBidInMaxAuctionBidsArray >= highestBidObj.bidAmount) {
      ...
      EAWinner = {
        EAWinningTeam: winner[0].bidTeam,
        EAWinnerBid: highestBidObj.bidAmount,
        highestBidObj: winnerObj,
      };
    } else {
      EAWinner = {
        EAWinningTeam: highestBidObj.bidTeam,
    ...
    EAWinner = {
      EAWinningTeam: winner[0].bidTeam,
      EAWinnerBid: winner[0].bidAmount,
      highestBidObj: winner[0],
    };
  }
  return EAWinner;
}
...
const updateLeaderBoardAfterNominationAuction = (room) => {
  ...
  nominatedBidsKeys.forEach((bidsId) => {
    ...
    const nominatedTeamData = currentLeaderBoard[nominatedTeam].filter(
      (pData) => pData.auctionId !== nominatedAuctionBids[bidsId].auctionId
    );
    ...
    const data = {
      ...
      artMovement: nominatedAuctionBids[bidsId].artMovement,
      artMovementId: nominatedAuctionBids[bidsId].artMovementId,
      name: nominatedAuctionBids[bidsId].name,
      player: {
        playerName: nominatedAuctionBids[bidsId].player.playerName,
        teamName: nominatedAuctionBids[bidsId].player.teamName,
        playerId: nominatedAuctionBids[bidsId].player.playerId,
        hostCode: nominatedAuctionBids[bidsId].player.hostCode,
        teamColor: nominatedAuctionBids[bidsId].player.teamColor,
      },
      bidAmount: parseInt(nominatedAuctionBids[bidsId].bidAmount, 10),
      bidAt: nominatedAuctionBids[bidsId].bidAt,
      bidTeam: nominatedAuctionBids[bidsId].bidTeam,
